/**
 * Автоотправка фильтров в шапке yii\grid\GridView. Фильтры типа input, select, date
 */
class inputAutoSend {
    static init() {
        if ($("div").is("#grid_id")) {
            ...
            let timerFilter = new timerForFilter(TIME_FOR_FILTER, inputAutoSend.applyFilter);
            let keyCode = 0;
    ...
    static applyFilter()
    {
        $("#grid_id").yiiGridView("inputAutoSend.applyFilter");
    }
}
...
/**
 * Если автокомплит в шапке yii\grid\GridView предложит варианты в выпадающем списке,
 * то эта функция позволяет отправить выбранное щелчком мыши
 * @param event
 * @param ui
 * @param attribute
 */
